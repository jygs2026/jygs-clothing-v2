...
export function InventoryScreen() {
  ...
  /** Per-size deltas, keyed "productId:size". Applied over the catalogue. */
    ...
    () =>
      CATALOGUE.map((item) => ({
        ...
        stock: item.stock.map((row) => ({
          ...row,
          onHand: Math.max(0, row.onHand + (adjusted[`${item.id}:${row.size}`] ?? 0)),
        })),
      })),
    [adjusted]
  );
  ...
  function adjust(item: CatalogueItem, size: Size, by: number) {
    setAdjusted((prev) => {
      const key = `${item.id}:${size}`;
      const current = item.stock.find((row) => row.size === size)?.onHand ?? 0;
      // Never below zero: a shelf cannot hold minus two.
      if (current + by < 0) return prev;
      return { ...prev, [key]: (prev[key] ?? 0) + by };
    });
  }
          ...
          <div className="flex items-center gap-1">
            {row.stock.map((entry) => (
              <span
                ...
                className={cn(
                  "flex h-7 min-w-9 flex-col items-center justify-center rounded border px-1 text-[10px] leading-none",
                  entry.onHand === 0
  ...
  return (
    <div className="mx-auto max-w-[1240px] px-4 py-7 sm:px-6 sm:py-9">
      ...
      <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-6">
        ...
        <AdminStatCard label="Stock at cost" value={money(stats.value)} detail="What it cost to make" />
        <AdminStatCard
          label="Unsaved"
          value={changes}
      ...
      <AdminPanel className="mt-5">
        ...
        <DataTable
          ...
          card={{
            ...
            fields: ["onHand", "committed", "available", "sizes"],
            wide: ["sizes"],
          }}
        />
      </AdminPanel>
